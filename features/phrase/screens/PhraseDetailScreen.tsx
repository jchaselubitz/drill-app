...
export default function PhraseDetailScreen() {
  ...
  // Fetch SrsCards for linked translations
  useEffect(() => {
    if (linkedTranslations.length === 0) {
      setSrsCards([]);
      return;
    }

    const translationIds = linkedTranslations.map((item) => item.translation.id);
    const subscription = db.collections
            ...
            >
              <View style={styles.statsContainer}>
                {srsCards.map((card) => {
                  ...
                  const nextReviewDate = new Date(card.dueAt);
                  ...
                  return (
                    ...
                    >
                      <Text style={[styles.statDirection, { color: colors.text }]}>
                        {directionLabel}
                      </Text>
